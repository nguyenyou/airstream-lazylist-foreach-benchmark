...
const [, , beforePath, afterPath, ...resultPaths] = process.argv
...
rows.sort((left, right) => left.runtime.localeCompare(right.runtime) || left.size - right.size)

console.log("## Runtime benchmark")
console.log("")
console.log("| Runtime | LazyList size | Before median | After median | Less time | Throughput | Checksum |")
console.log("|:---|---:|---:|---:|---:|---:|:---:|")
for (const row of rows) {
  console.log(`| ${row.runtime} | ${row.size} | ${row.beforeMs.toFixed(2)} ms | ${row.afterMs.toFixed(2)} ms | ${row.lessTime.toFixed(1)}% | ${row.throughput.toFixed(2)}x | ${row.checksumMatches ? "yes" : "NO"} |`)
}
